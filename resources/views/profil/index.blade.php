...
<section class="flex flex-col items-center text-center px-6 mt-4">
    ...
    <h1 class="mt-5 text-4xl font-bold text-white tracking-tight">{{ $user->username ?? 'User' }}</h1>
    <div class="mt-8">
        <p class="text-xs uppercase tracking-[0.35em] text-gray-500">Nama Karakter</p>
        <p class="mt-2 text-xl font-semibold text-indigo-300">{{ $user->character_name ?? 'Belum diisi' }}</p>
    </div>
    <div class="mt-8 max-w-md">
        <p class="text-xs uppercase tracking-[0.35em] text-gray-500">Tentang Saya</p>
        <p class="mt-3 text-gray-300 italic leading-relaxed">{{ $user->bio ?? 'Belum ada bio...' }}</p>
    </div>
</section>
...
<nav class="fixed bottom-0 w-full px-6 pb-8 pt-4 z-20">
    <div class="bg-indigo-600/40 backdrop-blur-xl h-16 rounded-2xl flex justify-around items-center shadow-[0_10px_40px_rgba(79,70,229,0.4)] border border-white/20">
        ...
        <a href="{{ route('profil') }}" class="p-3 bg-white/20 shadow-inner rounded-xl transition"><img src="https://img.icons8.com/material-outlined/24/ffffff/user.png" class="w-6 h-6"/></a>
    </div>
</nav>
